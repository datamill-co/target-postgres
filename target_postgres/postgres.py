...
class MillisLoggingConnection(LoggingConnection):
    ...
    def filter(self, msg, curs):
        return "MillisLoggingConnection: {} millis spent executing: {}".format(
            int((time.monotonic() - curs.timestamp) * 1000), msg
        )
...
class PostgresTarget(SQLInterface):
        ...
        **kwargs
        ...
        self.LOGGER.info(
            "PostgresTarget created with established connection: `{}`, PostgreSQL schema: `{}`".format(
                connection.dsn, postgres_schema
            )
        )
    ...
    def _update_schemas_0_to_1(self, cur):
        ...
        for mapped_name, raw_json in cur.fetchall():
            ...
            if metadata and metadata.get("schema_version", 0) == 0:
                self.LOGGER.info(
                    "Migrating `{}` from schema_version 0 to 1".format(mapped_name)
                )
    ...
    def _update_schemas_1_to_2(self, cur):
        ...
        for mapped_name, raw_json in cur.fetchall():
            ...
            if (
                metadata
                and metadata.get("schema_version", 0) == 1
                and metadata.get("table_mappings")
            ):
                self.LOGGER.info(
                    "Migrating root_table `{}` children from schema_version 1 to 2".format(
                        mapped_name
                    )
                )
                ...
                for mapping in metadata.get("table_mappings"):
                    ...
                    self.LOGGER.info(
                        "Migrating `{}` (`{}`) from schema_version 1 to 2".format(
    ...
    def metrics_tags(self):
        return {
            "database": self.conn.get_dsn_parameters().get("dbname", None),
        ...
        with self.conn.cursor() as cur:
            try:
                ...
                root_table_name = self.add_table_mapping_helper(
                    (stream_buffer.stream,), self.table_mapping_cache
                )["to"]
                ...
                if current_table_schema:
                    ...
                    if set(stream_buffer.key_properties) != set(
                    ...
                    for key_property in stream_buffer.key_properties:
                        ...
                        ) = self.fetch_column_from_path(
                            (key_property,), current_table_schema
                        )
                        if self.json_schema_to_sql_type(
                            remote_column_schema
                        ) != self.json_schema_to_sql_type(
                            stream_buffer.schema["properties"][key_property]
                        ):
                            raise PostgresError(
                                (
                                    ...
                                    json_schema.get_type(
                                        current_table_schema["schema"]["properties"][
                                            key_property
                                        ]
                                    ),
                                    json_schema.get_type(
                ...
                target_table_version = (
                    current_table_version or stream_buffer.max_version
                )
                ...
                root_table_name = self.table_prefix + stream_buffer.stream
                if (
                    current_table_version is not None
                    and stream_buffer.max_version is not None
                ):
                    if stream_buffer.max_version < current_table_version:
                        self.LOGGER.warning(
                            "{} - Records from an earlier table version detected.".format(
                                stream_buffer.stream
                            )
                        )
        ...
        with self.conn.cursor() as cur:
            try:
                ...
                if not current_table_schema:
                    ...
                    current_table_schema.get("version") is not None
                    and current_table_schema.get("version") >= version
                ):
                    self.LOGGER.warning(
                    ...
                    for versioned_table_name in map(lambda x: x[0], cur.fetchall()):
                        table_name = (
                            root_table_name
                            + versioned_table_name[len(versioned_root_table) :]
                        )
                        ...
                        """
                                ...
                                stream_table_old=sql.Identifier(
                                    table_name + SEPARATOR + "old"
                                ),
    ...
    def _validate_identifier(self, identifier):
        ...
        if self.IDENTIFIER_FIELD_LENGTH < len(identifier):
            raise PostgresError(
                "Length of identifier must be less than or equal to {}. Got {} for `{}`".format(
                    self.IDENTIFIER_FIELD_LENGTH, len(identifier), identifier
                )
            )
        ...
        if not re.match(r"^[a-z0-9_$]+$", identifier):
            raise PostgresError(
                "Identifier must only contain lower case letters, numbers, underscores, or dollar signs. Got `{}` for `{}`".format(
                    re.findall(r"[^0-9]", "1234a567")[0], identifier
                )
            )
    ...
    def add_table(self, cur, path, name, metadata):
        ...
        create_table_sql = sql.SQL("CREATE TABLE {}.{}").format(
            sql.Identifier(self.postgres_schema), sql.Identifier(name)
        )
    ...
    def _get_update_sql(
        self, target_table_name, temp_table_name, key_properties, columns, subkeys
    ):
        full_table_name = sql.SQL("{}.{}").format(
            sql.Identifier(self.postgres_schema), sql.Identifier(target_table_name)
        )
        full_temp_table_name = sql.SQL("{}.{}").format(
            sql.Identifier(self.postgres_schema), sql.Identifier(temp_table_name)
        )
        ...
        for pk in key_properties:
            ...
            pk_temp_select_list.append(
                sql.SQL("{}.{}").format(full_temp_table_name, pk_identifier)
            )
        ...
        distinct_order_by = sql.SQL(" ORDER BY {}, {}.{} DESC").format(
            pk_temp_select, full_temp_table_name, sql.Identifier(singer.SEQUENCE)
        )

        if len(subkeys) > 0:
            pk_temp_subkey_select_list = []
            for pk in key_properties + subkeys:
                pk_temp_subkey_select_list.append(
                    sql.SQL("{}.{}").format(full_temp_table_name, sql.Identifier(pk))
                )
    ...
    def persist_csv_rows(self, cur, remote_schema, temp_table_name, columns, csv_rows):
        ...
        subkeys = list(
            filter(lambda header: re.match(pattern, header) is not None, columns)
        )
    ...
    def write_table_batch(self, cur, table_batch, metadata):
        ...
        ## Persist csv rows
        self.persist_csv_rows(
            cur, remote_schema, target_table_name, csv_headers, csv_rows
        )
    ...
    def _get_table_metadata(self, cur, table_name):
        cur.execute(
            ...
            SELECT EXISTS (
                ...
                WHERE schemaname = {} AND
                      tablename = {});
        """
            ).format(sql.Literal(self.postgres_schema), sql.Literal(table_name))
        )
        ...
        cur.execute(
            sql.SQL(
                "SELECT description FROM pg_description WHERE objoid = {}::regclass;"
            ).format(sql.Literal('"{}"."{}"'.format(self.postgres_schema, table_name)))
        )
    ...
    def add_column_mapping(self, cur, table_name, from_path, to_name, mapped_schema):
        ...
        mapping = {"type": json_schema.get_type(mapped_schema), "from": from_path}
        ...
        if self.add_upsert_indexes:
            ...
            for column_name__or__path in schema["schema"]["properties"].keys():
                ...
                if len(column_path) == 1:
                    if column_path[0] == "_sdc_sequence" or column_path[0].startswith(
                        "_sdc_level_"
                    ):
    ...
    def is_table_empty(self, cur, table_name):
        cur.execute(
            sql.SQL("SELECT EXISTS (SELECT * FROM {}.{});").format(
                sql.Identifier(self.postgres_schema), sql.Identifier(table_name)
            )
        )
    ...
    def __get_table_schema(self, cur, name):
        ...
        """
            ).format(sql.Literal(self.postgres_schema), sql.Literal(name))
        )
    ...
    def sql_type_to_json_schema(self, sql_type, is_nullable):
        ...
        if sql_type == "timestamp with time zone":
            ...
            raise PostgresError(
                "Unsupported type `{}` in existing target table".format(sql_type)
            )
    ...
    def json_schema_to_sql_type(self, schema):
        ...
        if "format" in schema and schema["format"] == "date-time" and _type == "string":
